...
export type CommunityStatus = 'active-sales' | 'presale' | 'under-development'
...
export type AppData = {
  ...
  addCommunity: (community: Omit<Community, 'id' | 'thumbnail' | 'planIds' | 'status'>) => void
...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const addCommunity = React.useCallback((newCommunity: Omit<Community, 'id' | 'thumbnail' | 'planIds' | 'status'>) => {
  ...
  const updateCommunity = React.useCallback((id: string, updates: Partial<Community>) => {
    setCommunities(prev => prev.map(c => c.id === id ? { ...c, ...updates } : c))
  }, [])

  const value: AppData = { communities, plans, lots, addCommunity, updateCommunity }
